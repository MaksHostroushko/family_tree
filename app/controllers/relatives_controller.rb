class RelativesController < ApplicationController
  ...
  def index
    ...
    @user = current_user
    @relatives = @user.relatives.paginate(page: params[:page], per_page: 3) if @user.present?
    @relatives = @relatives.where(id: CategoryRelative.where(category_id: params[:filter]).pluck(:relative_id)).paginate(page: params[:page], per_page: 3) if params[:filter].present?
    @relatives = Relative.search(params[:search]).order(created_at: :asc).paginate(page: params[:page], per_page: 3)  if params[:search]
